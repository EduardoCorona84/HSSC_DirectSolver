function Y = BHSS1D_apply_nsym(K11,K12,K21,K22,B_dn,B_up,I_dn,I_up,X)
%
...
%        block matrix with non-symmetric HSS1D elements; it is specified by parameters
...
Vec1 = zeros(n1,size(X,2)); 
Vec1(I_up{1,1},:) = X1;
% Y11 = K11*Vec = A11*X1
Y11 = HSS1D_apply_submatrix_nsym(K11,B_dn{1,1},B_up{1,1},I_dn{1,1},I_up{1,1},Vec1,1);

% Vec(I_up,:) = X2, zero elsewhere
Vec1 = zeros(k1+r2,size(X,2)); 
Vec1(I_up{1,2},:) = X2;
% Y12 = K12*Vec = A12*X2
Y12 = HSS1D_apply_submatrix_nsym(K12,B_dn{1,2},B_up{1,2},I_dn{1,2},I_up{1,2},Vec1,1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Vec(I_up,:) = X1, zero elsewhere
Vec2 = zeros(k2+r1,size(X,2));
Vec2(I_up{2,1},:) = X1;
% Y21 = K21*Vec = A21*X1
Y21 = HSS1D_apply_submatrix_nsym(K21,B_dn{2,1},B_up{2,1},I_dn{2,1},I_up{2,1},Vec2,1);

% Vec(I_up,:) = X2, zero elsewhere
Vec2 = zeros(n2,size(X,2)); 
Vec2(I_up{2,2},:) = X2;
% Y22 = K22*Vec = A22*X2
Y22 = HSS1D_apply_submatrix_nsym(K22,B_dn{2,2},B_up{2,2},I_dn{2,2},I_up{2,2},Vec2,1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Build Y

Y = zeros(k1+k2,size(X,2));
