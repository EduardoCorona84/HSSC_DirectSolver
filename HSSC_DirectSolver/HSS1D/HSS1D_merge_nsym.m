function [EHSS,X_sort,J] = HSS1D_merge_nsym(E1,E2,X,lev,acc,opts)
%
...
%
%    DESCRIPTION:
%        This function merges two non-symmetric HSS structures encoded in E1 and E2 into a (2 x 2)
...
if (n1>3 && n2>3) 
    ...
    while cond == 1 && (n1>3 && n2>3)
        ...
        cond = (lf_small1>0 || lf_small2>0) || (lf_lson1>0 || lf_lson2>0);
    
        if cond == 1
            E1 = HSS1D_reducelev_nsym(E1,acc);
            E2 = HSS1D_reducelev_nsym(E2,acc); 
        end
    end
end
...
numlev1 = zeros(1,E1{BOX.LEVEL,n1}+1); 
numlev2 = zeros(1,E2{BOX.LEVEL,n2}+1); 
    ...
    for ibox = 3:(-1):2
        if size(E,2)>3
            ...
            if ison1>0 && ison2>0
                ...
                E{BOX.I_SKDN,ibox} = [E{BOX.I_SKDN,ison1} E{BOX.I_SKDN,ison2}];
            else
                E{BOX.I_SKUP,ibox} = E{BOX.I_SKUP,max(ison1,ison2)};
                E{BOX.I_SKDN,ibox} = E{BOX.I_SKDN,max(ison1,ison2)};
            end 
        else
            E{BOX.I_SKUP,ibox} = E{BOX.END1,ibox} - 1 + (1:E{BOX.END2,ibox});
            E{BOX.I_SKDN,ibox} = E{BOX.END1,ibox} - 1 + (1:E{BOX.END2,ibox});
        end
        ...
        E{BOX.J_DN,ibox} = 1:E{BOX.NSKEL,ibox}; 
        E{BOX.T_DN,ibox} = zeros(0,E{BOX.NSKEL,ibox});
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% We assume the trees for E1 and E2 have the same structure, since they
% correspond to two sides of the same interface. Hence, we only need to
% match them and concatenate the corresponding matrices. 

% Match leaves 

Endopts1 = zeros(1,n1); Endopts2 = Endopts1; leaf = false(1,n1); 
...
for ibox = nboxes:-1:1
    if (E{BOX.C1,ibox}<=0 & E{BOX.C2,ibox}<=0)
        ...
        L1 = [eye(k1)  E1{BOX.T_DN,ibox1}].'; L1(E1{BOX.J_DN,ibox1},:) = L1; 
        L2 = [eye(k2)  E2{BOX.T_DN,ibox2}].'; L2(E2{BOX.J_DN,ibox2},:) = L2;
        
        R = [R1 zeros(k1,m2) ; zeros(k2,m1) R2]; 
        L = [L1 zeros(m1,k2) ; zeros(m2,k1) L2];
        ...
        ison2 = E{BOX.C2,ibox};
        if (ison1>0 & ison2>0)
            indskel_out = [E{BOX.I_SKUP,ison1},E{BOX.I_SKUP,ison2}];
            indskel_in  = [E{BOX.I_SKDN,ison1},E{BOX.I_SKDN,ison2}];
            ...
            indskel_out = E{BOX.I_SKUP,ison};
            indskel_in = E{BOX.I_SKDN,ison};
        ...
        if (ison1>0 & ison2>0)
            ...
            L_c1 = E{BOX.LAMBDA,ison1}; L_c2 = E{BOX.LAMBDA,ison2};
            ...
            % Interpolation matrix factors depending on box order previous
            % to merge
            
            if E{10,ison1}<E{10,ison2}
                R11 = R1(:,1:k1_1);     L11 = L1(1:k1_1,:);
                R12 = R1(:,k1_1+1:end); L21 = L1(k1_1+1:end,:);
            else
                R11 = R1(:,k2_1+1:end); L11 = L1(k2_1+1:end,:);
                R12 = R1(:,1:k2_1);     L21 = L1(1:k2_1,:);
            end
            
            if E{11,ison1}<E{11,ison2}
                R21 = R2(:,1:k1_2);     L12 = L2(1:k1_2,:); 
                R22 = R2(:,k1_2+1:end); L22 = L2(k1_2+1:end,:);
            else
                R21 = R2(:,k2_2+1:end); L12 = L2(k2_2+1:end,:);
                R22 = R2(:,1:k2_2);     L22 = L2(1:k2_2,:);
            end
            
            R = [R11*R_c1(1:k1_1,:) R12*R_c2(1:k2_1,:) ; R21*R_c1(k1_1+1:end,:) R22*R_c2(k2_1+1:end,:)];
            L = [L_c1(:,1:k1_1)*L11 L_c1(:,k1_1+1:end)*L12 ; L_c2(:,1:k2_1)*L21 L_c2(:,k2_1+1:end)*L22];
            ...
            R_c = E{BOX.RHO,ison}; L_c = E{BOX.LAMBDA,ison}; 
            R = [R1*R_c(1:k1_1,:) ; R2*R_c(k1_1+1:end,:)];
            L = [L_c(:,1:k1_1)*L1 L_c(:,k1_1+1:end)*L2];
         end
        
        % Sibling Interactions
        if (ison1>0 & ison2>0)
            B11 = E1{BOX.M_SIB,E{10,ison1}}; B12 = E2{BOX.M_SIB,E{11,ison1}};
            B21 = E1{BOX.M_SIB,E{10,ison2}}; B22 = E2{BOX.M_SIB,E{11,ison2}};
            
            %E{BOX.M_SIB,ison1} = [W11' W21']*[B1 zeros(size(B1,1),size(B2,2)) ; ...
            %              zeros(size(B2,1),size(B1,2)) B2]*[W12 ; W22];
            E{BOX.M_SIB,ison1} = L_c1(:,1:k1_1)*B11*R_c2(1:k2_1,:) + L_c1(:,k1_1+1:end)*B12*R_c2(k2_1+1:end,:);
            E{BOX.M_SIB,ison2} = L_c2(:,1:k2_1)*B21*R_c1(1:k1_1,:) + L_c2(:,k2_1+1:end)*B22*R_c1(k1_1+1:end,:); 
            
            %Clean Auxiliary info for sons
            E{BOX.RHO,ison1} = []; E{BOX.RHO,ison2} = [];
            E{BOX.LAMBDA,ison1} = []; E{BOX.LAMBDA,ison2} = [];
            E{10,ison1} = []; E{11,ison1} = []; 
            E{10,ison2} = []; E{11,ison2} = [];
            clear R_c1 R_c2 R11 R12 R21 R22 L_c1 L_c2 L11 L12 L21 L22;
        else
    ...
    if ibox>1
        ...
        k = size(Tout,1); 
        E{BOX.KSKEL,ibox} = k; 
        E{BOX.J_UP,ibox} = Jout; %J
        E{BOX.I_SKUP,ibox} = indskel_out(Jout(1:k)); %I_sk
        E{BOX.T_UP,ibox} = Tout; % T
        E{BOX.J_DN,ibox} = Jin; %J
        E{BOX.I_SKDN,ibox} = indskel_in(Jin(1:k)); %I_sk
        E{BOX.T_DN,ibox} = Tin; % T
    
        E{BOX.RHO,ibox} = R(:,Jout(1:k));
        E{BOX.LAMBDA,ibox} = L(Jin(1:k),:);
    end
end
