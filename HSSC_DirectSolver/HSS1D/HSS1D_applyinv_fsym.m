function xx = HSS1D_applyinv_fsym(K_HSS,HSSinv,FTinv,ff)
%
...
%
%    DESCRIPTION:
%        This function uses the HSSinv structure {HSSinv,FTinv} of a full-symmetric HSS1D matrix A to
...
% The upwards pass - collect all "outgoing fields" on FIELDS{1,ibox}. 
% This corresponds to multiplication by Rinv (\tilde{R}) matrices. 
for ibox = nboxes:(-1):2
...
x_top = FTinv*f_top;
FIELDS{2,2} = x_top(1:size(HSSinv{BOX.Rinv,2},1),:);
FIELDS{2,3} = x_top((size(HSSinv{BOX.Rinv,2},1)+1):end,:);
...
for ibox = 2:nboxes
  if ( (K_HSS{BOX.C1,ibox} <= 0) & (K_HSS{BOX.C2,ibox} <= 0)) % ibox is a leaf
    ind = K_HSS{BOX.END1,ibox} - 1 + (1:K_HSS{BOX.END2,ibox});
    xx(ind,:) = HSSinv{BOX.Rinv,ibox}.'*FIELDS{2,ibox} + HSSinv{BOX.Dinv,ibox}*ff(ind,:);
  elseif ( (K_HSS{BOX.C1,ibox} >  0) & (K_HSS{BOX.C2,ibox} <= 0)) % ibox has one son - the left one.
    ison1           = K_HSS{BOX.C1,ibox};
    FIELDS{2,ison1} = HSSinv{BOX.Rinv,ibox}.'*FIELDS{2,ibox} + ...
                      HSSinv{BOX.Dinv,ibox}*FIELDS{1,ison1};
  elseif ( (K_HSS{BOX.C1,ibox} <= 0) & (K_HSS{BOX.C2,ibox} >  0)) % ibox has one son - the right one.
    ison2           = K_HSS{BOX.C2,ibox};
    FIELDS{2,ison2} = HSSinv{BOX.Rinv,ibox}.'*FIELDS{2,ibox} + ...
                      HSSinv{BOX.Dinv,ibox}*FIELDS{1,ison2};
  elseif ( (K_HSS{BOX.C1,ibox} >  0) & (K_HSS{BOX.C2,ibox} >  0)) % ibox has two sons.
    ison1  = K_HSS{BOX.C1,ibox};
    ison2  = K_HSS{BOX.C2,ibox};
    k1     = K_HSS{BOX.KSKEL,ison1};
    k2     = K_HSS{BOX.KSKEL,ison2};
    x_long = HSSinv{BOX.Rinv,ibox}.'*FIELDS{2,ibox} + HSSinv{BOX.Dinv,ibox}*[FIELDS{1,ison1};FIELDS{1,ison2}];
