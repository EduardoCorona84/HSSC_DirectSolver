function uu = HSS1D_apply_rect_nsym(K_HSS,qq,dim)
%
...
for ibox = nboxes:(-1):2
  ...
  if ( (K_HSS{BOX.C1,ibox}<=0) && (K_HSS{BOX.C2,ibox}<=0) )
    ...
    FIELDS{2,ibox} = zeros(length(K_HSS{BOX.I_SKDN,ibox}),num);
  end
end
...
% uu is M x num
uu = zeros(K_HSS{17,1},num);
for ibox = nboxes:(-1):2
  if ( (K_HSS{BOX.C1,ibox}<=0) && (K_HSS{BOX.C2,ibox}<=0) )
      ind_out       = dim*K_HSS{BOX.END1,ibox} - dim + (1:dim*K_HSS{BOX.END2,ibox});
      ind_in        = dim*K_HSS{16,ibox} - dim + (1:dim*K_HSS{17,ibox});
    uu(ind_in,:) = LOCAL_skel_eval(K_HSS{BOX.T_DN,ibox},K_HSS{BOX.J_DN,ibox},FIELDS{2,ibox}) + ...
                K_HSS{BOX.M_SELF,ibox}*qq(ind_out,:);
  end
end
