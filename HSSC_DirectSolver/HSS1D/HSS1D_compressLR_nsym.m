function K_HSS = HSS1D_compressLR_nsym(U,V,TREE,acc)
%
...
for ibox = nboxes:(-1):1
  if ( (K_HSS{BOX.C1,ibox}>0) && (K_HSS{BOX.C2,ibox}>0) )
    ...
    K_HSS{BOX.M_SIB,ison1} = U(K_HSS{BOX.I_SKDN,ison1},:)*(V(K_HSS{BOX.I_SKUP,ison2},:).');
    K_HSS{BOX.M_SIB,ison2} = U(K_HSS{BOX.I_SKDN,ison2},:)*(V(K_HSS{BOX.I_SKUP,ison1},:).'); 
