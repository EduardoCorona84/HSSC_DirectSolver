function uu = HSS1D_apply_submatrix_fsym(K_HSS,Box_dn,Box_up,I_dn,I_up,qq,dim)
%
...
%
%    DESCRIPTION:
%        This function applies a submatrix of a full-symmetric HSS1D matrix to a vector : uu = K(
%        I_dn,I_up) qq(I_up)
%
%    INPUT:
...
for ibox = nboxes:(-1):2
  ...
  if ( (K_HSS{BOX.C1,ibox}<=0) && (K_HSS{BOX.C2,ibox}<=0) )
    ...
    FIELDS{2,ibox} = zeros(length(K_HSS{BOX.I_SKUP,ibox}),q2);
  end
end
...
if Box_up(2)==true && Box_dn(3)==true
    if ( (K_HSS{BOX.C1,2}>0) || (K_HSS{BOX.C2,2}>0) ) || size(K_HSS{BOX.M_SIB,3},2)==K_HSS{BOX.KSKEL,2}
        ...
        FIELDS{2,3} = K_HSS{BOX.M_SIB,2}(I_srcup{2},:).'*FIELDS{1,2};
        
    end
else
    FIELDS{2,3} = zeros(length(K_HSS{BOX.I_SKUP,3}),q2);
end
      ...
      if (K_HSS{BOX.C1,ibox}>0)
          ...
          if (K_HSS{BOX.C2,ibox}>0)
              ...
              if Box_dn(ison1)==true
                  ...
                  FIELDS{2,ison1} = zeros(length(K_HSS{BOX.I_SKUP,ison1}),q2); 
              end
          end
      else
          ison2   = K_HSS{BOX.C2,ibox};
          if Box_dn(ison2)==true
              ...
              FIELDS{2,ison2} = zeros(length(K_HSS{BOX.I_SKUP,ison2}),q2);  
          end
      end
  end
end
  ...
  if ( (K_HSS{BOX.C1,ibox}<=0) && (K_HSS{BOX.C2,ibox}<=0) )
    ...
    if Box_dn(ibox)==true
        if (sum(I_srcdn{ibox})==K_HSS{BOX.END2,ibox})
            ...
            if Box_up(ibox)==true
            uu(ind(I_srcdn{ibox}),:) = LOCAL_skel_eval_submatrix(K_HSS{BOX.T_UP,ibox},K_HSS{BOX.J_UP,ibox},I_srcdn{ibox},FIELDS{2,ibox}) + ...
