function Y = HSS2D_applyinv_fsym(INFO_TREE,A_HSS,X,params)
%
...
if ~strcmp(Fopts_def,'dense') && ~strcmp(Fopts_def,'block')    
...
for lev = depth:-1:1
    ...
    for i = 1:nblev
        ...
        psi = HSS2D_applyFinv_fsym(FI,q,k,Fopts);    
        ...
        elseif strcmp(Eopts,'HSS')
            %JE = A_HSS(Nbox).JE; 
            phi_up{Nbox}(JE,:) = HSS1D_apply_fsym(E,tmp(JE,:));  
        end
        clear tmp;
    end
    clear k J T FI E JE
...
for lev = 0:depth
    ...
    for i = 1:nblev
        ...
        if lev == 0
            ...
            elseif strcmp(Eopts,'HSS')
                %JE = A_HSS(Nbox).JE; 
                tmp(JE,:) = HSS1D_apply_fsym(E,phi_dn{Nbox}(JE,:));  
            end 
