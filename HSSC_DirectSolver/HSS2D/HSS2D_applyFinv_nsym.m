function Y = HSS2D_applyFinv_nsym(FI,V,k,opts)
%
...
elseif strcmp(opts,'HSS')
    ...
    % HSS apply of F_{rs}^{-1}
    psi = HSS1D_apply_nsym(FI.rs_Inv,V_rs); 
    % LR apply of F_r2s
    eta = LR_apply(FI.rs2sk,psi); 

    % Y defined at skeleton points I_sk
    % HSS apply of S_{rs}^{-1}
    Y_sk = HSS1D_apply_nsym(FI.SrsInv,(V_sk - eta)); 

    % (II) Result for I_rs
    % LR apply of F_s2r
    lam =  LR_apply(FI.sk2rs,Y_sk);   

    % HSS apply of F_{rs}^{-1}
    Y_rs = HSS1D_apply_nsym(FI.rs_Inv,(V_rs - lam)); 
