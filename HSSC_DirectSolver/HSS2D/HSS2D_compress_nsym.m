function [A_HSS] = HSS2D_compress_nsym(INFO_TREE,params)
%
...
for lev = 0:INFO_TREE.depth
    ...
    for i = 1:numlev
        ...
        if INFO_TREE.BOX(Nbox).child(1) == 0
            ...
            if lev>lev_HSS
                Bself = Kernel_Eval(Source(I_src,:),Source(I_src,:),params); 
            else
                Bself  = HSS1D_compress_nsym('green',Source(I_src,:),nmax,params.acc,par); 
            end
            ...
            if lev>lev_HSS
                ...
                B21 = Kernel_Eval(X2,X1,params);
            else
                X12 = [X1 ; X2];
                T12 = LOCAL_tree_2branch(size(X1,1),size(X2,1),nmax); 
                
                B12  = HSS1D_compress_nsym('green',X12,T12,params.acc,par,'offd'); 
                B21 = [];
            end
...
function TREE = LOCAL_tree_2branch(n1,n2,nbox_max)
global BOX 
